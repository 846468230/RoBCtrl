...
def hash_state_action(s_t, a_t):
	key = s_t[0]
	base = 179424673
	for i in range(s_t[1].added_edges.shape[1]):
		key = (key * base + s_t[1].added_edges[0, i]) % base
		key = (key * base + s_t[1].added_edges[1, i]) % base
	...
	def add_list(self, list_st, list_at, list_rt, list_sp, list_term, t):
		for i in range(len(list_st)):
			if list_sp is None:
				...
				sp = list_sp[i]
			self.add(list_st[i], list_at[i], list_rt[i], sp, list_term[i], t[i])

	def sample(self, batch_size, t = None):
		if t is None:
			if self.mem_cells[-1].sub_list[0].count<=batch_size:
				times,list_st, list_at, list_rt, list_s_primes, list_term = [],[],[],[],[],[]
				for i in range(batch_size):
					t = np.random.randint(self.n_steps)
					temp1,temp2,temp3, temp4, temp5 = self.mem_cells[t].sample(1)
					times.append(t)
					list_st.append(temp1)
					list_at.append(temp2)
					list_rt.append(temp3)
					list_s_primes.append(temp4)
					list_term.append(temp5)
				return times,list_st, list_at, list_rt, list_s_primes, list_term
			else:
				t = np.random.randint(self.n_steps)
